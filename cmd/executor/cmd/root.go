...
func addKanikoOptionsFlags() {
	...
	RootCmd.PersistentFlags().VarP(&opts.RegistriesCertificates, "registry-certificate", "", "Use the provided CA certificate for TLS communication with the given registry. Expected format is 'my.registry.url=/path/to/the/server/certificate'.")
	opts.RegistriesClientCerts = make(map[string]string)
	RootCmd.PersistentFlags().VarP(&opts.RegistriesClientCerts, "registry-client-cert", "", "Use the provided certificate and key for TLS client communication with the given registry. Expected format is 'my.registry.url=/path/to/the/server/client.pem'. Note that a corresponding client-key.pem is also required in the same directory (-key will be added automatically).")
